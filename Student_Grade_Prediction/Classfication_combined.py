...
random_forest_fpr, random_forest_tpr, threshold = roc_curve(
    y_test, y_pred_random_forest)
auc_random_forest = auc(random_forest_fpr, random_forest_tpr)

kernel_svm_fpr, kernel_svm_tpr, threshold = roc_curve(
    y_test, y_pred_kernel_svm)
auc_kernel_svm = auc(kernel_svm_fpr, kernel_svm_tpr)

decision_tree_fpr, decision_tree_tpr, threshold = roc_curve(
...
plt.plot(naive_fpr, naive_fpr, linestyle='-',
         label='Naive Bayes (auc = %0.3f)' % auc_naive)
plt.plot(logistic_fpr, logistic_tpr, marker='.',
         label='Logistic (auc = %0.3f)' % auc_logistic)
plt.plot(svm_fpr, svm_tpr, linestyle='-', label='SVM (auc = %0.3f)' % auc_svm)
plt.plot(kernel_svm_fpr, kernel_svm_tpr, linestyle='-',
         label='Kernel SVM (auc = %0.3f)' % auc_kernel_svm)
plt.plot(decision_tree_fpr, decision_tree_tpr, linestyle='-',
         label='Decision Tree (auc = %0.3f)' % auc_decision_tree)
plt.plot(random_forest_fpr, random_forest_tpr, linestyle='-',
         label='Random Forest(auc = %0.3f)' % auc_random_forest)
